...
int particleFilter(unsigned char * I, int IszX, int IszY, int Nfr, int * seed, int Nparticles) {
    ...
    for (k = 1; k < Nfr; k++) {
        ...
        err = clEnqueueNDRangeKernel(cmd_queue, kernel_likelihood, 1, NULL, global_work, NULL, 0, 0, 0);
        clFinish(cmd_queue);
        if (err != CL_SUCCESS) {
        ...
        //KERNEL FUNCTION CALL
        err = clEnqueueNDRangeKernel(cmd_queue, kernel_sum, 1, NULL, global_work, NULL, 0, 0, 0);
        clFinish(cmd_queue);
        if (err != CL_SUCCESS) {
        ...
        //KERNEL FUNCTION CALL
        err = clEnqueueNDRangeKernel(cmd_queue, kernel_normalize_weights, 1, NULL, global_work, NULL, 0, 0, 0);
        clFinish(cmd_queue);
        if (err != CL_SUCCESS) {
	...
	//	printf("arrayX[%d]:%f | arrayY[%d]:%f | weights[%d]:%f\n",
 	//		x, arrayX[x], x, arrayY[x], x, weights[x]); 
        ...
        //KERNEL FUNCTION CALL
        err = clEnqueueNDRangeKernel(cmd_queue, kernel_find_index, 1, NULL, global_work, NULL, 0, 0, 0);
        clFinish(cmd_queue);
        if (err != CL_SUCCESS) {
